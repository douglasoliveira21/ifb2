"""assistencia_social indicator category
...
def upgrade() -> None:
    op.execute("ALTER TYPE indicator_category ADD VALUE IF NOT EXISTS 'ASSISTENCIA_SOCIAL'")
...
def downgrade() -> None:
    op.execute(
        ...
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM indicator_definitions WHERE category = 'ASSISTENCIA_SOCIAL') THEN
                RAISE EXCEPTION 'Não é possível reverter: existem indicator_definitions com category=ASSISTENCIA_SOCIAL. Apague-os antes do downgrade.';
            END IF;
        END $$;
        """
    )
