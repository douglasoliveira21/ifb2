"""pessoas_com_deficiencia indicator category
...
def upgrade() -> None:
    op.execute("ALTER TYPE indicator_category ADD VALUE IF NOT EXISTS 'PESSOAS_COM_DEFICIENCIA'")
...
def downgrade() -> None:
    op.execute(
        ...
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM indicator_definitions WHERE category = 'PESSOAS_COM_DEFICIENCIA') THEN
                RAISE EXCEPTION 'Não é possível reverter: existem indicator_definitions com category=PESSOAS_COM_DEFICIENCIA. Apague-os antes do downgrade.';
            END IF;
        END $$;
        """
    )
