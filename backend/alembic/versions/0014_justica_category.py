...
def upgrade() -> None:
    op.execute("ALTER TYPE indicator_category ADD VALUE IF NOT EXISTS 'JUSTICA'")
...
def downgrade() -> None:
    op.execute(
        ...
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM indicator_definitions WHERE category = 'JUSTICA') THEN
                RAISE EXCEPTION 'Não é possível reverter: existem indicator_definitions com category=JUSTICA. Apague-os antes do downgrade.';
            END IF;
        END $$;
        """
    )
    ...
    op.execute(
        ...
        CREATE TYPE indicator_category AS ENUM (
            ...
            'MULHERES', 'CRIANCAS', 'IDOSOS', 'DESENVOLVIMENTO_REGIONAL'
        )
        """
    )
    op.execute(
