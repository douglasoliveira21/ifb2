...
def upgrade() -> None:
    op.execute("ALTER TYPE indicator_category ADD VALUE IF NOT EXISTS 'INDUSTRIA'")
...
def downgrade() -> None:
    op.execute(
        ...
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM indicator_definitions WHERE category = 'INDUSTRIA') THEN
                RAISE EXCEPTION 'Não é possível reverter: existem indicator_definitions com category=INDUSTRIA. Apague-os antes do downgrade.';
            END IF;
        END $$;
        """
    )
    ...
    op.execute(
        ...
        CREATE TYPE indicator_category AS ENUM (
            ...
            'HABITACAO', 'SANEAMENTO', 'AGRICULTURA'
        )
        """
    )
    op.execute(
