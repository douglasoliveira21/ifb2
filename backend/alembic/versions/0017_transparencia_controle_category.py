"""transparencia_controle indicator category
...
def upgrade() -> None:
    op.execute("ALTER TYPE indicator_category ADD VALUE IF NOT EXISTS 'TRANSPARENCIA_CONTROLE'")
...
def downgrade() -> None:
    op.execute(
        ...
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM indicator_definitions WHERE category = 'TRANSPARENCIA_CONTROLE') THEN
                RAISE EXCEPTION 'Não é possível reverter: existem indicator_definitions com category=TRANSPARENCIA_CONTROLE. Apague-os antes do downgrade.';
            END IF;
        END $$;
        """
    )
    ...
    op.execute(
        ...
        CREATE TYPE indicator_category AS ENUM (
            ...
            'GESTAO_PUBLICA', 'COMERCIO_EXTERIOR'
        )
        """
    )
    op.execute(
